...
* Предыдущая проблема может всплыть в первозданном виде, если Вы впишете всю известную информацию о научном руководителе, и она будет слишком
  длинной, чтобы влезть в одну строчку. В качестве решения проблемы можно писать только степень и звание, как и требуется в оригинальном шаблоне аннотации. (Issue #1)
...
* Ширина индексной части в оглавлении вбита как константа, что плохо работает, в частности, с приложениями, когда шрифт не Times. (Issue #2)
* Некоторые вещи, например, число страниц, являются гиперссылками (под управлением пакета `hyperref`), хотя не должны. (Issue #3)
...
```
Возможное решение --- откатиться на более раннюю версию (2.8 или 2.9). (Issue #4)
* В ГОСТ запрещены переносы в названиях разделов как в оглавлении, так и в тексте. В тексте это требование выполняется, а в оглавлении --- нет. (Issue #5)
* В ГОСТ нумерация рисунков, таблиц и т.д. в основном тексте идет глобально, а в приложениях идет по приложениям (рисунок А.26). Данное переключение не реализовано. (Issue #6)
