...
* Ширина индексной части в оглавлении вбита как константа, что плохо работает, в частности, с приложениями, когда шрифт не Times.
* Некоторые вещи, например, число страниц, являются гиперссылками (под управлением пакета `hyperref`), хотя не должны.
...
* В ГОСТ запрещены переносы в названиях разделов как в оглавлении, так и в тексте. В тексте это требование выполняется, а в оглавлении --- нет.
* В ГОСТ нумерация рисунков, таблиц и т.д. в основном тексте идет глобально, а в приложениях идет по приложениям (рисунок А.26). Данное переключение не реализовано.
